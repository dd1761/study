...
x1_train, x1_test, x2_train, x2_test, x3_train, x3_test, y_train, y_test = train_test_split(
    x1_datasets, x2_datasets, x3_datasets, y, train_size=0.7, random_state=1234          # x데이터를 2개이상 집어넣어도 순서가 바뀌지 않으면 가능하다.
)
...
model = Model(inputs=[input1, input2, input3], outputs=last_output) # 모델 선언, 모델의 시작은 input1, input2이고 마지막은 last_output이다.

...
from tensorflow.keras.layers import Dense, Input, Dropout, Conv2D, Flatten, MaxPool2D, LSTM, Conv1D
import numpy as np
...
x_train = x_train.reshape(331, 5, 2)         #x_train을 4차원으로 변환   
x_test = x_test.reshape(111, 5, 2)            #x_test을 4차원으로 변환                   

model = Sequential()
model.add(Conv1D(64, 2, input_shape=(5,2)))
...
mcp = ModelCheckpoint(monitor='val_loss', mode='auto', verbose=1,
                    ...
                    #   filepath = path +'MCP/keras30_ModelCheckPoint3.hdf5'
                      filepath = filepath + 'k51_03_' + date + '_' + filename
                      )
